...
function identityFile() {
  // HOME is honored so a developer can use an isolated account and tests can
  // prove enrollment without touching the caller's real identity location.
  return resolve(process.env.HOME || homedir(), ".config", "team-skills", "age", "identity.txt");
}
...
function decrypt(source, format, execute) {
  ...
  if (execute === execFileSync && !existsSync(identityFile())) {
    fail("No enrolled age identity. Use team-skills vault enroll --from <private-identity-file>.");
  }
  const inputFormat = sopsFormat(format);
  try {
    return Buffer.from(execute("sops", ["--decrypt", "--input-type", inputFormat, "--output-type", inputFormat, source], {
      encoding: null,
      stdio: "pipe",
      env: { ...process.env, SOPS_AGE_KEY_FILE: identityFile() },
    }));
  } catch (error) {
...
function atomicWrite(destination, bytes) {
  ...
  const temporary = resolve(directory, `.${basename(destination)}.vault-${process.pid}-${randomUUID()}.tmp`);
  let fd;
  try {
...
function sopsFormat(format) {
  const normalized = String(format || "binary").toLowerCase();
  const formats = { dotenv: "dotenv", json: "json", yaml: "yaml", yml: "yaml", ini: "ini", binary: "binary", text: "binary" };
  if (!formats[normalized]) fail(`Unsupported vault format: ${format}`);
  return formats[normalized];
}

function configPath(root) {
  const config = projectPath(root, ".sops.yaml", "SOPS configuration");
  if (!existsSync(config.absolute) || isSymlink(config.absolute)) fail("A committed non-symlink .sops.yaml is required to initialize a vault entry.");
  return config;
}

export function init(path, root = process.cwd(), log = console.log, execute = execFileSync) {
  try {
    const resolvedRoot = resolve(root);
    const source = sourcePath(resolvedRoot, path);
    if (existsSync(source.absolute)) fail(`Refusing to replace existing vault source: ${source.relative}`);
    const config = configPath(resolvedRoot);
    const manifest = loadManifest(resolvedRoot);
    const entry = Object.values(manifest.entries).find((candidate) => candidate.source === source.relative);
    const format = sopsFormat(entry?.format || "binary");
    const encrypted = Buffer.from(execute("sops", [
      "--encrypt", "--config", config.absolute, "--filename-override", source.relative,
      "--input-type", format, "--output-type", format, "/dev/null",
    ], { cwd: resolvedRoot, encoding: null, stdio: "pipe", env: { ...process.env } }));
    if (!encrypted.length) fail("SOPS returned no encrypted output.");
    atomicWrite(source.absolute, encrypted);
    log(`✓ initialized encrypted vault source: ${source.relative}`);
    return true;
  } catch (error) {
    log(`✗ ${error.message}`);
    return false;
...
export function updatekeys(root = process.cwd(), log = console.log, execute = execFileSync) {
  try {
    const resolvedRoot = resolve(root);
    const manifest = loadManifest(resolvedRoot);
    const config = configPath(resolvedRoot);
    for (const entry of Object.values(manifest.entries)) {
      const { source } = ensureSafeEntry(resolvedRoot, entry);
      execute("sops", ["--config", config.absolute, "updatekeys", "--yes", "--input-type", sopsFormat(entry.format), source.absolute], {
        cwd: resolvedRoot,
        encoding: null,
        stdio: "pipe",
        env: { ...process.env, SOPS_AGE_KEY_FILE: identityFile() },
    ...
    log(`✓ updated recipients for ${Object.keys(manifest.entries).length} vault entr${Object.keys(manifest.entries).length === 1 ? "y" : "ies"}`);
    return true;
  } catch (error) {
    if (error?.code === "ENOENT") log(`✗ Official SOPS is unavailable. Install it for this machine: ${SOPS_INSTALL_URL}`);
    else log("✗ SOPS could not update vault recipients. Verify committed .sops.yaml and the enrolled identity without printing secret content.");
    return false;
  }
}

export function edit(name, root = process.cwd(), log = console.log, execute = execFileSync) {
  try {
    if (!process.stdin.isTTY || !process.stdout.isTTY) fail("vault edit requires an interactive terminal; it is intentionally unavailable to non-interactive agents and CI.");
    const resolvedRoot = resolve(root);
    const { entry } = entryFor(name, resolvedRoot);
    const { source } = ensureSafeEntry(resolvedRoot, entry);
    const config = configPath(resolvedRoot);
    if (!existsSync(identityFile())) fail("No enrolled age identity. Use team-skills vault enroll --from <private-identity-file>.");
    execute("sops", ["--config", config.absolute, "--input-type", sopsFormat(entry.format), "--output-type", sopsFormat(entry.format), source.absolute], {
      cwd: resolvedRoot,
      stdio: "inherit",
      env: { ...process.env, SOPS_AGE_KEY_FILE: identityFile() },
    });
    log(`✓ ${entry.name}: encrypted vault source updated`);
    return true;
  } catch (error) {
    if (error?.code === "ENOENT") log(`✗ Official SOPS is unavailable. Install it for this machine: ${SOPS_INSTALL_URL}`);
    else log(`✗ ${error.message || "SOPS editing failed without changing the vault entry."}`);
    return false;
  }
}
...
export function doctor(root = process.cwd(), log = console.log, execute = execFileSync) {
  try {
    ...
    const identityReady = existsSync(identityFile());
...
export function enroll(from, log = console.log) {
  try {
    ...
    if (existsSync(identityFile())) fail("An age identity is already enrolled; do not overwrite it implicitly.");
    copyFileSync(source, identityFile());
...
export function check(name, root = process.cwd(), log = console.log, execute = execFileSync) {
  try {
    ...
    decrypt(source.absolute, entry.format, execute);
...
export function materialize(name, requestedDestination, root = process.cwd(), log = console.log, execute = execFileSync) {
  try {
    ...
    const bytes = decrypt(source.absolute, entry.format, execute);
