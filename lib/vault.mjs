...
function decrypt(source, execute) {
  // Tests inject an in-memory fake executor. Real commands require the packaged
  // binary and an enrolled identity; never silently fall back to PATH.
  if (execute !== execFileSync) {
    return Buffer.from(execute("sops", ["--decrypt", source], { encoding: null, stdio: "pipe" }));
  }
  const sops = process.env.TEAM_SKILLS_SOPS_BINARY;
  if (!sops) fail("No packaged SOPS binary is available. Production releases must provide the pinned package binary.");
  if (!existsSync(IDENTITY_FILE)) fail("No enrolled age identity. Use team-skills vault enroll --from <private-identity-file>.");
...
export function doctor(root = process.cwd(), log = console.log) {
  try {
    const manifest = loadManifest(root);
    const sops = process.env.TEAM_SKILLS_SOPS_BINARY;
    const ready = Boolean(sops && existsSync(sops) && existsSync(IDENTITY_FILE));
    log(`  manifest         ✓ ${Object.keys(manifest.entries).length} entry(s)`);
    log(`  age identity     ${existsSync(IDENTITY_FILE) ? "✓ enrolled" : "✗ not enrolled"}`);
    log(`  packaged SOPS    ${sops && existsSync(sops) ? "✓ available" : "✗ unavailable"}`);
    return ready;
  } catch (error) {
    log(`✗ ${error.message}`);
    return false;
