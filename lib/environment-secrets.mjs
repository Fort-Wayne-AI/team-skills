...
SECURITY
  ...
  • A single .env file with one DOTENV_PRIVATE_KEY serves all environments.
`);
}
...
export function run(args, root, log = console.log, execute = execFileSync) {
  // Strip a leading "--" separator so both forms work:
  //   team-skills env run -- npm run dev   (documented)
  //   team-skills env run npm run dev      (convenient)
