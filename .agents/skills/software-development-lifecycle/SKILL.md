...
8. **Pass CI and review.** Confirm CI runs for the PR and reruns after each pushed commit. Address failures and review feedback, then re-review changed code.
...
3. Confirm `main` is green and the release commit is identified.
4. Create the release manually using the repository's release mechanism.
5. Deploy manually to the intended environment; do not infer deployment from merge alone.
6. Verify production health and key user journeys, record the deployed version, and be prepared to roll back or remediate.
